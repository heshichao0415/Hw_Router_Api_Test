...
class GetGameList_auth:
    doexcel_ob = DoExcel(contants.case_file, 'GetGameList')
    log_ob = MyLog(__name__).my_log()

    @pytest.mark.parametrize('case', doexcel_ob.get_cases())
    def test_auth_GetGameList(self, before_after_request, case):
        ...
        try:
            ...
            DoExcel(contants.case_file, 'GetGameList').write_case(row, result, 'pass')
            self.log_ob.debug("成功用例回写EXCEL成功")

        except:
            DoExcel(contants.case_file, 'GetGameList').write_case(row, result, 'failed')
            self.log_ob.debug("失败用例回写EXCEL成功")
            assert self.result['code'] == eval(case.expected)['code']

if __name__ == '__main__':
    GetGameList_auth().test_auth_GetGameList()